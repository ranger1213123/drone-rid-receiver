...
// ═══════════ Settings ═══════════
function loadSettings(){Api.get('/api/settings').then(function(s){if(s){['stWarnDist','stSevDist','stCritDist','stDebounce'].forEach(function(id){if(s[id]!=null)document.getElementById(id).value=s[id]})}}).catch(function(){})}
window.saveSettings=function(){var data={warn_distance:parseFloat(document.getElementById('stWarnDist').value),severe_distance:parseFloat(document.getElementById('stSevDist').value),critical_distance:parseFloat(document.getElementById('stCritDist').value),debounce_seconds:parseInt(document.getElementById('stDebounce').value)||60};Api.put('/api/settings',data).then(function(res){if(res.error){UI.toast(res.error,'error');return}UI.toast('设置已保存','ok')}).catch(catchErr('保存设置失败'))};
