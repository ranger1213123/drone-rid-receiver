...
status, j = test("csrf token", "GET", "/api/csrf-token")
...
status, j = test("login", "POST", "/login", {"username": "admin", "password": "admin123"}, expect_status=None)
...
status, j = test("csrf token (auth)", "GET", "/api/csrf-token")
if isinstance(j, dict) and j.get("token"):
    CSRF_TOKEN = j["token"]
...
status, j = test("status", "GET", "/api/status")
...
status, j = test("status paginated", "GET", "/api/status?page=1&per_page=10")
status, j = test("status incremental", "GET", "/api/status?since=2020-01-01T00:00:00")

status, j = test("stats dashboard", "GET", "/api/stats/dashboard")
if isinstance(j, dict):
...
status, j = test("list powerlines", "GET", "/api/powerlines")

# Create
status, j = test("create powerline", "POST", "/api/powerlines", {
    "name": "TEST_LINE_01",
...
if pl_id:
    status, lines = test("list powerlines (verify)", "GET", "/api/powerlines")
    ...
    # Edit (PUT)
    status, j = test("edit powerline", "PUT", f"/api/powerlines/{pl_id}", {
    ...
    # Delete
    status, j = test("delete powerline", "DELETE", f"/api/powerlines/{pl_id}")
    if isinstance(j, dict) and j.get("status") == "ok":
        print(f"       (deleted)")
...
status, j = test("list stations", "GET", "/api/stations")

# Create
status, j = test("create station", "POST", "/api/stations", {
...
# Edit (PUT)
status, j = test("edit station", "PUT", "/api/stations", {
    "name": "TEST_STATION_01",
...
# Verify edit
status, stations = test("list stations (verify)", "GET", "/api/stations")
...
# Delete
status, j = test("delete station", "DELETE", "/api/stations", {"name": "TEST_STATION_01"})

# ═══════════════════════════════════════════════════════════════
section("6. Users CRUD (GET/POST/PUT/DELETE)")
# ═══════════════════════════════════════════════════════════════

status, j = test("list users", "GET", "/api/users")

# Create
status, j = test("create user", "POST", "/api/users", {
...
# Edit (PUT)
status, j = test("edit user", "PUT", "/api/users", {
    "username": "test_user_api",
    "role": "user",
...
# Verify edit
status, users = test("list users (verify)", "GET", "/api/users")
...
# Delete
status, j = test("delete user", "DELETE", "/api/users", {"username": "test_user_api"})
...
status, j = test("change password (wrong old)", "PUT", "/api/password", {
    "old_password": "wrong",
    "new_password": "newpass123",
}, expect_status=403)

# Reset password (admin only)
status, j = test("reset user password", "POST", "/api/users/admin/reset-password", {
...
status, j = test("list personnel", "GET", "/api/personnel")

# Create (using correct field name "station_name")
status, j = test("create personnel", "POST", "/api/personnel", {
...
status, j = test("list personnel (filtered)", "GET", "/api/personnel?station=TEST_STATION_01")

# Delete
if personnel_id:
    status, j = test("delete personnel", "DELETE", "/api/personnel", {"id": personnel_id})

# ═══════════════════════════════════════════════════════════════
section("9. Alerts & Export")
# ═══════════════════════════════════════════════════════════════

status, j = test("alerts history", "GET", "/api/alerts/history")
status, j = test("alerts history (filtered)", "GET", "/api/alerts/history?level=warning&limit=10")
status, j = test("alerts history (ack)", "GET", "/api/alerts/history?acknowledged=0")
status, j = test("alerts export", "GET", "/api/alerts/export")
status, j = test("drones export", "GET", "/api/drones/export")

# ═══════════════════════════════════════════════════════════════
section("10. Trajectories")
# ═══════════════════════════════════════════════════════════════

status, j = test("trajectory list", "GET", "/api/trajectories")
if isinstance(j, dict):
...
status, j = test("list licenses", "GET", "/api/licenses")
status, j = test("create license", "POST", "/api/licenses", {
...
# Edit license
if tenant_id:
    status, j = test("edit license", "PUT", "/api/licenses", {
        "id": tenant_id,
...
# Delete (soft-delete)
if tenant_id:
    status, j = test("delete license", "DELETE", "/api/licenses", {"id": tenant_id})

# ═══════════════════════════════════════════════════════════════
section("12. Device Provisioning")
# ═══════════════════════════════════════════════════════════════

status, j = test("list devices", "GET", "/api/devices")

status, j = test("provision device", "POST", "/api/devices/provision", {
...
# Revoke
if isinstance(j, dict) and j.get("status") == "ok":
    status, j = test("revoke device", "POST", f"/api/devices/TEST_DEV_PROV/revoke", None, expect_status=None)
    print(f"       (revoke: {json.dumps(j, ensure_ascii=False, default=str)[:120]})")

# Delete device
status, j = test("delete device", "DELETE", "/api/devices/TEST_DEV_PROV")

# ═══════════════════════════════════════════════════════════════
section("13. Tenant Self-Service")
# ═══════════════════════════════════════════════════════════════

status, j = test("tenant info", "GET", "/api/tenant/info")

# Registration (public, no auth needed — but needs valid key)
status, j = test("register stations (public)", "GET", "/api/register/stations?key=INVALID-KEY")

# ═══════════════════════════════════════════════════════════════
section("14. Audit & Settings")
# ═══════════════════════════════════════════════════════════════

status, j = test("audit logs", "GET", "/api/audit")
status, j = test("audit logs (limited)", "GET", "/api/audit?limit=10")
status, j = test("settings get", "GET", "/api/settings")
status, j = test("settings update", "PUT", "/api/settings", {
...
CSRF_TOKEN = None
status, j = test("CSRF rejection", "POST", "/api/stations", {
...
status, j = test("logout", "GET", "/logout", expect_status=None)

# After logout, protected endpoints should return 401
status, j = test("status (no auth)", "GET", "/api/status", expect_status=401)
if status == 401:
...
status, j = test("re-login for CSRF check", "POST", "/login", {"username": "admin", "password": "admin123"}, expect_status=None)
if status in (200, 302):
    print(f"       (login OK)")

# Get fresh CSRF token
status, j = test("csrf token (fresh)", "GET", "/api/csrf-token")
if isinstance(j, dict) and j.get("token"):
    CSRF_TOKEN = j["token"]
...
_saved = CSRF_TOKEN
CSRF_TOKEN = None
status, j = test("CSRF rejection (no token)", "POST", "/api/stations", {"name": "CSRF_TEST_DUMMY"}, expect_status=403)
CSRF_TOKEN = _saved
if status == 403:
    print(f"       (CSRF correctly rejected)")
    PASS += 1
